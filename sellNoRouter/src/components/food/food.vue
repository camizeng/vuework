    ...
    <div v-show="showFlag" class="food" ref="foodInfo">
      <div class="food-content">
        ...
        <div class="rating">
          ...
          <!-- 评价头 -->
          <ratingselect @typeSelect="ratingTypeSelect" @conToggle="contentToggle"
          :selectType="ratSelType" :onlyContent="onlyContent" :desc="desc" :ratings="food.ratings"></ratingselect>
          <!-- 评价内容 -->
          <div class="rating-wrapper">
            <ul v-show="food.ratings && food.ratings.length">
              <li v-show="needShow(rating.rateType,rating.text)" v-for="rating in food.ratings" :key="rating.id" class="rating-item border-1px">
                ...
                <p class="text">
                  <span :class="{'icon-thumb_up':rating.rateType===0, 'icon-thumb_down':rating.rateType===1}" />{{rating.text}}
                </p>
              </li>
            </ul>
...
export default {
  ...
  methods: {
    ...
    needShow(type, text) {
      ...
      if (this.ratSelType === ALL) {
        return true;
      } else {
        return type === this.ratSelType;
      }
    },
    ratingTypeSelect(type) {
      console.log('ratingtype.select', type);
      this.ratSelType = type;
      this.$nextTick(() => {
        this.scroll.refresh();
      });
    },
